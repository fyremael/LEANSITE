...
private def escapeWith (attribute : Bool) (input : String) : String :=
  input.toList.foldl (fun out c =>
    match c with
    | '&' => out ++ "&amp;"
    | '<' => out ++ "&lt;"
    | '>' => out ++ "&gt;"
    | '"' => if attribute then out ++ "&quot;" else out.push c
    | '\'' => if attribute then out ++ "&#39;" else out.push c
    | _ => out.push c) ""
...
private def renderAttribute (attribute : String × String) : String :=
  s!" {attribute.1}=\"{escapeAttribute attribute.2}\""
